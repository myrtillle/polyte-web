...
const EditRewardModal = ({ isOpen, onClose, onSave, initialReward }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      fetchRewardTypes();
      if (initialReward) {
        setRewardName(initialReward.reward_name || '');
        setRewardDescription(initialReward.reward_description || '');
        setPolyPointsRequired(initialReward.poly_points_required || '');
        setSelectedTypeId(initialReward.reward_type_id || '');
      }
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg w-96">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={handleSubmit} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Save Changes</button>
        </div>
      </div>
    </div>
  );
